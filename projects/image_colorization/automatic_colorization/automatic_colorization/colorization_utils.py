...
def colorize_image(model, filename, input_path, output_path):
    image_path = input_path + filename
    image = load_image(image_path)

    result_path = output_path + 'result_' + filename
    result = model.predict(image)

    imsave(result_path, result.astype('uint8'))


def colorize_images(model, input_path, output_path):
    for filename in os.listdir(input_path):
        colorize_image(model, filename, input_path, output_path)
