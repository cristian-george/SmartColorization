...
def visualize_lab(rgb_image):
    ...
    for i in range(0, n_rows):
        for j in range(0, n_cols):
            ...
            imsave('images/figures/result_Places365_val_00033087_{}{}.bmp'.format(i, j), img)

            axes[i][j].set_title(title)
            axes[i][j].imshow(img)
            axes[i][j].axis('off')

        plt.savefig('images/figures/result_Places365_val_00033087.svg', format='svg', dpi=1000)
        plt.show()


if __name__ == "__main__":
    img_path = 'images/results/result_Places365_val_00033087.jpg'
